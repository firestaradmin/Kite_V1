...
#include "esp_log.h"
static const char* TAG = "LCD_ST7789_114";

#if 1
#define _VENDOR "ZhongJingYuan"
#define _DESC "1.14 inch 135x240 IPS"



#define _RESOLUTION_SHORT   135
#define _RESOLUTION_LONG    240
...
DRAM_ATTR static LcdInitCmdSeq _lcd_init_cmds[] = {
    {0x11, {0}, 0},
    /* screen direction : Memory Data Access Control, MX=MV=1, MY=ML=MH=0, RGB=0 */
...
void lcd_init_st7789_114(LcdConfigure *config, LcdDevice *dev_dest)
{
...
static void _lcd_address_set(LcdDevice *dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    if (dev->config.direction == k_direction_clockwise_0)
    {
        ...
        _lcd_wr_data_16bit(dev, x1 + 52);
        _lcd_wr_data_16bit(dev, x2 + 52);
        _lcd_wr_reg(dev, 0x2b); // 行地址设置
        _lcd_wr_data_16bit(dev, y1 + 40);
        _lcd_wr_data_16bit(dev, y2 + 40);
        _lcd_wr_reg(dev, 0x2c); // 储存器写
    }
    else if (dev->config.direction == k_direction_clockwise_180)
    {
        _lcd_wr_reg(dev, 0x2a); // 列地址设置
        _lcd_wr_data_16bit(dev, x1 + 53);
        _lcd_wr_data_16bit(dev, x2 + 53);
        _lcd_wr_reg(dev, 0x2b); // 行地址设置
        _lcd_wr_data_16bit(dev, y1 + 40);
        _lcd_wr_data_16bit(dev, y2 + 40);
        _lcd_wr_reg(dev, 0x2c); // 储存器写
    ...
    {
        _lcd_wr_reg(dev, 0x2a); // 列地址设置
        _lcd_wr_data_16bit(dev, x1 + 40);
        _lcd_wr_data_16bit(dev, x2 + 40);
        _lcd_wr_reg(dev, 0x2b); // 行地址设置
        _lcd_wr_data_16bit(dev, y1 + 53);
        _lcd_wr_data_16bit(dev, y2 + 53);
        _lcd_wr_reg(dev, 0x2c); // 储存器写
    }
    else if (dev->config.direction == k_direction_clockwise_270)
    {
        _lcd_wr_reg(dev, 0x2a); // 列地址设置
        _lcd_wr_data_16bit(dev, x1 + 40);
        _lcd_wr_data_16bit(dev, x2 + 40);
        _lcd_wr_reg(dev, 0x2b); // 行地址设置
        _lcd_wr_data_16bit(dev, y1 + 52);
        _lcd_wr_data_16bit(dev, y2 + 52);
        _lcd_wr_reg(dev, 0x2c); // 储存器写
    }
    else
    {
