...
#include "esp_log.h"
static const char* TAG = "LCD_ST7735S_096";


#if 1
#define _VENDOR "ZhongJingYuan"
#define _DESC "0.96 inch 80x160 IPS"


#define _RESOLUTION_SHORT   80
#define _RESOLUTION_LONG    160
...
void lcd_init_st7735s_096(LcdConfigure *config, LcdDevice *dev_dest)
{
...
static void _lcd_address_set(LcdDevice *dev, uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2)
{
    if (dev->config.direction == k_direction_clockwise_0)
    {
        ...
        _lcd_wr_data_16bit(dev, x1 + 26);
        _lcd_wr_data_16bit(dev, x2 + 26);
        _lcd_wr_reg(dev, 0x2b); // 行地址设置
        _lcd_wr_data_16bit(dev, y1 + 1);
        _lcd_wr_data_16bit(dev, y2 + 1);
    ...
    {
        _lcd_wr_reg(dev, 0x2a); // 列地址设置
        _lcd_wr_data_16bit(dev, x1 + 1);
        _lcd_wr_data_16bit(dev, x2 + 1);
        _lcd_wr_reg(dev, 0x2b); // 行地址设置
        _lcd_wr_data_16bit(dev, y1 + 26);
        _lcd_wr_data_16bit(dev, y2 + 26);
        _lcd_wr_reg(dev, 0x2c); // 储存器写
    }
